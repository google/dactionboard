...
WITH
    ...
    SitelinksTable AS (
      ...
      FULL JOIN SitelinksAdGroupTable USING(campaign_id)
      GROUP BY 1
    ),
    /* Audience Block */
    ...
    TargetingCriteriaTable AS (
        ...
        FULL JOIN RemarketingTable  AS R USING(customer_id)
        FULL JOIN OptimizedTargetingCriteria AS O USING(customer_id)
        GROUP BY 1
    ),
